...
function initDatabase() {
    ...
    // Generate mock prices with multiple sources
    cardsDB.forEach(card => {
        const grades = ['ungraded', 'psa9', 'psa10', 'bgs9', 'bgs9.5', 'bgs10'];
        grades.forEach(grade => {
            const basePrice = generateMockPrice(card.rarity, grade);
            ...
            // PSA prices (for graded cards only)
            if (grade !== 'ungraded') {
                pricesDB.push({
                    card_id: card.id,
                    grade: grade,
                    language: 'en',
                    price: basePrice * (1 + Math.random() * 0.1), // Slight variation
                    currency: 'USD',
                    source: 'psa',
                    auctionCount: Math.floor(Math.random() * 50) + 5,
                    scraped_at: new Date().toISOString()
                });
            }
            ...
            pricesDB.push({
                ...
                currency: 'USD',
                source: 'ebay_sold',
                soldCount: Math.floor(Math.random() * 30) + 1,
                scraped_at: new Date().toISOString()
            });
            ...
            pricesDB.push({
                ...
                grade: grade,
                language: 'en',
                price: basePrice * (1.05 + Math.random() * 0.15),
                currency: 'USD',
                source: 'ebay_active',
...
// Multi-source price search (NEW)
app.get('/api/prices/search', async (req, res) => {
    try {
        const { q: cardName, number: cardNumber, grade = 'psa10', includeRaw = 'true' } = req.query;
        
        if (!cardName) {
            return res.status(400).json({ error: 'Card name required (use q parameter)' });
        }

        const { PriceAggregator } = require('../src/scrapers');
        const aggregator = new PriceAggregator();
        
        const grades = includeRaw === 'true' ? ['ungraded', 'psa9', 'psa10'] : [grade];
        
        const results = await aggregator.getCardPrices(cardName, cardNumber, { grades, language: 'en' });
...
function getCardPrices(cardId) {
    ...
    for (const grade of grades) {
        ...
        for (const lang of languages) {
            const gradePrices = pricesDB.filter(p => 
                p.card_id === cardId && p.grade === grade && p.language === lang
            );
            
            gradePrices.forEach(p => {
                if (lang === 'en') {
                    if (p.source === 'tcgplayer') prices[grade].en.tcgplayer = p;
                    if (p.source === 'psa') prices[grade].en.psa = p;
                    if (p.source === 'ebay_sold') prices[grade].en.ebay_sold = p;
                    if (p.source === 'ebay_active') prices[grade].en.ebay_active = p;
                } else if (lang === 'jp') {
                    if (p.source === 'yuyutei') prices[grade].jp.yuyutei = p;
                }
            });
        }
    }
